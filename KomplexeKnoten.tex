...
\begin{lemma}
    ...
    Es gilt
    \[
        \abs{\sigma_r^0(z_0, z_1, \dots, z_{n-1})}
...
\begin{proof}
    ...
    \noindent Zusammen mit der Aussage des vorherigen Korollars folgt nun die Behauptung:
    \[
        \begin{split}
            \sum_{r=0}^{n-1} \abs{ u_{0r} }
...
Schwierigkeiten bereitet also erneut die Norm der inversen Vandermonde-Matrix.
Wir betrachten im Folgenden die zwei Fälle $j=0$ und $j \in \{1, \dots, n-1\}$
gesondert.
...
\begin{lemma}
    ...
    \]
    Es gilt dann $\abs{\uptau_j} = n$ für alle $j = 0, \dots, n-1$.
\end{lemma}
\begin{proof}
...
\end{lemma}
\begin{proof}
    Mit $p(z) \defeq \sum_{k=0}^{n-1} z^n = \prod_{j=1}^{n-1} (z - z_j)$ gilt
    \[
        \begin{split}
