...
\begin{proof}
    ...
    Bezeichnen wir für $j, r = 0, \dots, n-1$ mit $u_{jr} \in \C$ erneut die
    Einträge von $V^{-1}$, so ist sofort ersichtlich, dass
    \[
        \begin{split}
...
% SECTION:OUTLIER
\section[Vandermonde-Matrizen aus $(n\!-\!1)$ Einheitswurzeln und einem Ausreißer]{Vandermonde-Matrizen aus \boldmath{$(n\!-\!1)$} Einheitswurzeln und einem Ausreißer}
...
\begin{remark}
    ...
    \begin{figure}[H]
        \centering
        \input{images/sigma_row_sum.pgf}
    ...
    \begin{figure}[H]
        \centering
        \input{images/matrix_row_sum.pgf}
...
bereiten.  Wir betrachten im Folgenden die $j$-te Zeile von
$\Vand{\delta}^{-1}$ für $j=0$ und $j \in \{1, \dots, n-1\}$ gesondert.
...
\begin{proof}
    ...
    dem Beweis von \lemmaref{inverse_outlier_vandermonde_first_row_abs_sum}
    erhalten wir
    \[
        \begin{split}
            \sum_{r=0}^{n-1} \abs{u_{0r}}^2
            &= \sum_{r=0}^{n-1} \abs{\Pi_0}^2 \abs{\sigma_{n-1-r}^{0}(z)}^2\\
...
\begin{proof}
    ...
    $\uptau_0 = n$.
    Tatsächlich gilt für $j = 1, \dots, n-1$
    \[
        \begin{split}
            \abs{\uptau_j}
            &= \abs{\uptau_j \cdot \w_n^{-(n-1)\cdot j}}
    ...
    Mit $p(z) \defeq \prod_{k=1}^{n-1} (z - \wn^k)$ können wir $\uptau_0 = p(1)$
    schreiben. Bereits im Beweis von \lemmaref{outlier_sigma_zero} haben wir gesehen, dass
    \[
...
\end{lemma}
\begin{proof}
    Mit $\wn \defeq e^{\frac{2 \pi i}{n}}$ und $p(z) \defeq \sum_{k=0}^{n-1} z^n = \prod_{j=1}^{n-1} (z - \wn^j)$ gilt
    \[
        \begin{split}
...
\begin{proof}
    ...
    \[
        \begin{split}
            ...
            &\overset{(\ref{eq:explicit_inverse_vandermonde})}=
                \abs{\Pi_j \cdot \sigma_{r}^j(-z)} \\
            &= \abs{\Pi_j \cdot \left( \frac{(1 - z_0) + (z_0 - z_j) z_j^r}{(1-z_j)} \right)} \\
            &= \abs{\frac{(1 - z_0) + (z_0 - z_j) z_j^r}{(z_j-z_0) \cdot \left((z_j - 1) (z_j - z_1) \dots (z_j - z_{n-1}) \right)}}\\
            &\eqdef \abs{\frac{(1 - z_0) + (z_0 - z_j) z_j^r}{(z_j-z_0) \cdot \uptau_j }}\\
            &= \abs{\frac{1}{\uptau_j} \left( \frac{1 - z_0}{z_j-z_0} - z_j^r \right)},
        \end{split}
    \]
    mit
    \[
    ...
    \[
        \begin{split}
            ...
            = \frac{1}{n^2} \left( \rho^2 + 1 - \rho \left( e^{i \cdot \left(\varphi - \frac{2\pi j r}{n}\right)} + e^{i \cdot \left(\frac{2 \pi j r } {n} - \varphi\right)} \right) \right)\\
    ...
    für die $j$-te Zeile von $V(\delta)^{-1}$
    \[
        \begin{split}
    ...
    Mit
    $\abs{1-z_0} = 2 \cdot \abs{ \sin \left( \pi \frac{\delta}{n} \right) }$
    und
    $\abs{z_j-z_0} = 2 \cdot \abs{ \sin \left( \pi \frac{j - \delta}{n} \right) } $
    folgt
    \[
        \rho = \abs{ \frac{\sin \left( \pi \frac{\delta}{n} \right)}{\sin \left( \pi \frac{j - \delta}{n} \right)} },
    \]
