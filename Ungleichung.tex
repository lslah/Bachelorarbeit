...
\begin{lemma}
    ...
    \begin{equation}
        ...
        \Vand{\alpha z}^{-1} = \Vand{z}^{-1} \cdot \diag{\alpha^0, \dots, \alpha^{n-1}}.
    \end{equation}
\end{lemma}
    ...
    \[
        \begin{split}
            ...
            &\overset{(\ref{eq:explicit_inverse_vandermonde})}{=}
                (-1)^{r} \Pi_j \sigma_{r}^{j}(\alpha z_0, \dots, \alpha z_{j-1}, \alpha z_{j+1}, \dots, \alpha z_{n-1})\\
            &\overset{(\ref{eq:elementary_symmetric_polynomials_const_multiplication})}{=}
                \alpha^{r} (-1)^{r} \Pi_j \sigma_{r}^{j}(z_0, \dots, z_{j-1}, z_{j+1}, \dots, z_{n-1})\\
            &= u_{jr} \alpha^{r}.
        \end{split}
    \]
    Wie behauptet, können wir also
    $\tilde{V}^{-1} = V^{-1} \cdot \diag{\alpha^{0}, \dots, \alpha^{n-1}}$ schreiben.

\end{proof}

% FIXME: Bad title for this section.
